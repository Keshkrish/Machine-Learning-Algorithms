...
# All the functions are the same as in Q2.iii
def k_poly(x,y,case):
    z=(np.dot(x,y)+1)**case
    return z
def k_radial(x,y,sigma):
...
#K=Kernel_poly(X,2) #radial or polynomial kernel can be called with any sigma or d
K=Kernal_radial(X,0.1)
...
#We define the cluster of each data point xi as the index of the maximum element in the ith row of the H matrix
clust=np.zeros(1000,dtype=int)
for i in range(1000):
    clust[i]=np.argmax(H[i,:])
